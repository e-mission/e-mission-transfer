...
def extract_and_copy(user_email, container):
    run(f"docker exec {container} bash -c 'source setup/activate.sh; ./e-mission-py.bash bin/debug/extract_timeline_for_day_range_and_user.py -e {user_email} 2020-01-01 2025-01-01 /tmp/emission_ind'")
    run(f"docker exec {container} bash -c 'tar -czvf /tmp/{user_email}.tar.gz /tmp/emission_ind*'")
    run("docker cp {container}:/tmp/{user_email}.tar.gz /tmp/".format(container=container, channel=channel))
...
if __name__ == '__main__':
    ...
    fc-stack_fc-analysis-server
    if not args.container:
        program = user_email.split("_")[0]
